...
import { useBagStore } from "@/stores/useBagStore"; import { useItemStore } from "@/stores/useItemStore"
import * as itemSvc from "@/services/itemService"; import ItemEditSheet from "@/components/item/ItemEditSheet.vue"
const route=useRoute(); const router=useRouter(); const bagStore=useBagStore(); const itemStore=useItemStore()
...
const categories = computed(() => { const s=new Set(itemStore.items.filter(i=>i.bagId===route.params.id).map(i=>i.category).filter(Boolean)); return [...s] })
const availableItems = computed(() => itemStore.items.filter(i=>i.bagId!==route.params.id))
onMounted(async () => { await Promise.all([bagStore.loadAll(), itemStore.loadItems()]) })
function togglePicked(id) { picked.value=picked.value.includes(id)?picked.value.filter(x=>x!==id):[...picked.value,id] }
async function confirmPick() {
  for (const id of picked.value) { await itemSvc.updateItem(id,{bagId:bag.value.id}) }
  await itemStore.loadItems(); picked.value=[]; showAdd.value=false
}
...
<div class="flex flex-col h-full bg-gray-50">
  ...
  <div class="flex-1 overflow-y-auto px-4 pb-4">
    <div class="flex items-center justify-between mt-3 mb-3"><span class="text-xs text-gray-400">{{ items.length }} 件物品</span></div>
    ...
    <div class="bg-gray-100 rounded-xl flex items-center gap-2 px-3 h-9 mb-3"><span class="text-xs text-gray-400">🔍</span><input v-model="searchQ" class="flex-1 bg-transparent border-none outline-none text-sm" placeholder="搜索本包物品..."/></div>
    ...
    <div @click="showAdd=true" class="mt-4 flex items-center gap-2 py-3 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-blue-300 text-blue-600 transition-all justify-center">
      <span class="text-sm font-bold">+</span><span class="text-xs font-medium">添加物品到此包</span></div>
  </div>
  ...
  <div v-if="showAdd && addTab==='pick'" class="fixed inset-0 z-50 flex flex-col justify-end bg-black/30" @click.self="showAdd=false">
    <div class="bg-white rounded-t-2xl max-h-[75vh] flex flex-col overflow-hidden">
      ...
      <div class="flex-1 overflow-y-auto px-4 py-3">
        <div v-if="availableItems.length===0" class="text-center py-6 text-sm text-gray-400">所有物品已在此包中</div>
  ...
  <ItemEditSheet v-if="showAdd && addTab==='create'" :show="true" :preset-bag-id="bag?.id||''" @close="showAdd=false;addTab='pick'" @created="onCreated"/>
</div>
</template>